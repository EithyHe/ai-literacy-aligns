...
# AI素养题项图聚类分析流程 (Graph-based Pipeline)
...
- **输入**: `items_master.csv` (`text_norm`列)  
- **方法**: OpenAI `text-embedding-3-large` (3072维)  
- **输出**:  
  - `emb_base.npy` (原始嵌入, n×3072)
...
- **目的**: 增强同一 construct 题项的相似性，提高嵌入质量  
- **输出**:  
  - `emb_contrastive.npy` (增强嵌入)  
  - `proj_head.pt` (投影头模型)
...
- **输出**:  
  - `neighbors_graph.parquet` (题项相似度表)  
...
- **方法**: 测试4种图聚类方法：  
  1. `knn_kmeans`: k-NN图 + 谱嵌入 + KMeans (谱聚类变体)  
  2. `similarity_spectral`: 相似度图 + 谱聚类 (社区发现)  
  3. `graph_embedding`: 图嵌入 + KMeans  
  4. `dbscan`: 密度聚类  
- **参数**:  
  - k范围: [2, 15]  
  - 相似度阈值: 0.7  
...
#### 6.2 LLM自动标注
- **输入**: `graph_top_items.csv`  
- **方法**: GPT-4o-mini 分析代表题项 → 生成：  
  - `label`: 聚类主题标签 (2-5词)  
  - `rationale`: 一句话解释  
